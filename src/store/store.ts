import { ActionListener as ConcreteActionListener, ActionType, ActionTypeMap } from './actions'
import { State } from './state'

/**
 * A very simple redux-unlike store. Public and anyone can register as dispatcher. Tested usage practice is all
 * the redux bad-practices: 
 * 
...
export interface Store {
  /**
   * this is similar to store.dispatch in the sense that anybody from user facing dispatch simple action objects
   * against the store hen user performs something.
   */
  dispatch<T extends ActionType>(a: ActionTypeMap[T]): void
  /**
   * half store.subscribe() and half a redux-sage. this is the dispatch order:  `[...this.listeners.beforeWrite,
   * ...this.listeners.beforeReadOnly, ...this.listeners.afterWrite, ...this.listeners.afterReadOnly]`. listeners
   * can listen only  to one kind of action .
   */
...
/**
 * Dispatcher is like Reducer but with more freedom - receive the action and the entire state and mutates it if
 * it needs so. There are different modes on how a listener is notified when the state change and this is
 * declared with property [[listenerType]]
 */
export interface ActionListener<
  ...
  id: ActionListenerId
  /**
   * declares the mode in which this listener is notified. Some listeners can be notified before or after the
   * "dispatch" moment and can declare that they will "mutate" (write) the state. So ones could be more like redux
   * reducers, and other could have a role more similar to redux sagas...
   */
