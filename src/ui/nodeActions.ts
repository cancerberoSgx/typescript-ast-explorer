...
export function buildNodeActions(store: Store) {
  ...
  const container: blessed.Widgets.ListbarElement = grid.set(...gridPosition, blessed.listbar, {
    style: { ...buttonStyle },
    ...scrollableOptions,
    border: 'line',
    // padding: 1,
  ...
  container.on('click', (a, b) => {
    if (container) container.$.__originalHeight = container.height
    container.$.__collapsed = true
    container.height = 2
    screen.render()
  })
  return container
