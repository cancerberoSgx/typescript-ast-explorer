...
export function renderer(this: LayoutElement, coords: { xl: number; xi: number; yl: number; yi: number }) {
  ...
  const iterator: LayoutIterator = (el, i) => {
    ...
    if (!last) {
      ...
      // row).
      if (!(el.options && el.options.style && el.options.style.display!=='block') && el.position.left + el.width <= width) {
    ...
    if (this.style.overflow === 'hidden' && (el.position.top + el.height > height || el.getLines() && el.getLines().length > height)) {
